            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    @if ($page == $paginator->currentPage())
                        <span class="border-t mr-2 bg-red text-white hover:bg-red hover:text-white border-b border-l border-brand-light px-3 py-2 bg-brand-light no-underline">{{ $page }}</span>
                    @else
                        <a class="border-t hover:bg-red hover:text-white bg-white mr-2 border-b border-l border-brand-light px-3 py-2 hover:bg-brand-light text-black no-underline" href="{{ $url }}">{{ $page }}</a>
                    @endif
                @endforeach
            @endif
        @endforeach
