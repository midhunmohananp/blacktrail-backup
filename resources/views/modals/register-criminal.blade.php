<register-criminal inline-template>
	<modal name="add-criminal" height="auto">
		<div class="ml-4 p-4 w-full bg-white " class="	">
			<form @submit.prevent="registerCriminal" class="font-basic">
				<h3>Register Criminal</h3>
				<div class="mt-4">
					<label for="name" class="block uppercase tracking-wide text-grey-darker text-xs font-bold mb-2">Criminal's Name</label>
					<input v-model="form.registerers_name" type="text" value="{{ $displayName }} " class="bg-grey-lighter w-3/4 mb-2 p-2 leading-normal" id="pin" name="pin" autocomplete="name" placeholder="Criminal Name" value="{{ old('name') }}" required>
				</div>

				<div class="mb-2">
					<label for="name" class="block uppercase tracking-wide text-grey-darker text-xs font-bold mb-2">Aka</label>
					<input type="text" v-model="form.contact_number" class="bg-grey-lighter w-3/4 mb-2 p-2 leading-normal" id="pin" name="pin" autocomplete="name" placeholder="Alias" value="{{  auth()->user()->phone_number }}" required>
				</div>	
				...
				<div class="mb-2 ">
					<button type="submit" class="p-2 bg-blue w-3/4 font-bold text-white">Register Criminal</button>
				</div>
			</form>
		</div>
	</modal>
